...
# Keep the user's provider/plugin configuration, but isolate global tui.json so
# an installed copy of this plugin cannot shadow the local build with the same ID.
if [[ "$GLOBAL_CONFIG_DIR" != "$TEST_CONFIG_DIR" && -d "$GLOBAL_CONFIG_DIR" ]]; then
  shopt -s dotglob nullglob
  for source in "$GLOBAL_CONFIG_DIR"/*; do
    name="$(basename "$source")"
    [[ "$name" == "tui.json" || "$name" == "tui.jsonc" ]] && continue
